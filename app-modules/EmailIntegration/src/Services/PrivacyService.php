...
final readonly class PrivacyService
{
    ...
    public function defaultTierForUser(User $user): EmailPrivacyTier
    {
        ...
        return $user->currentTeam->default_email_sharing_tier;
    ...
    private function isProtected(Email $email): bool
    {
        ...
        $protectedEmails = ProtectedRecipient::where('team_id', $teamId)
            ->where('type', 'email')
            ->pluck('value')
            ->map(fn ($v) => strtolower($v));

        $protectedDomains = ProtectedRecipient::where('team_id', $teamId)
            ->where('type', 'domain')
            ->pluck('value')
            ->map(fn ($v) => strtolower($v));
